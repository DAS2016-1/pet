...
from pet.exceptions import VCSException, FileNotFound
...
class Subversion(VCS):
    ...
    def _list(self, path):
        ...
        if path not in self._cache:
            entries = (svn.ra.svn_ra_get_dir2(self.ra, path,
                       self.rev, svn.core.SVN_DIRENT_KIND |
    ...
    def changed_named_trees(self, session, named_trees_by_package):
        ...
        # We assume that all named_trees for a package were
        ...
        for package, nts in named_trees_by_package.items():
            ...
            for nt in nts:
                if nt.type == 'tag':
                    ...
                    raise Exception(
                        "unknown named_tree type (type={0}, name={1})"
                        .format(nt.type, nt.name))
            ...
            if package_name not in tags:
                ...
                if len(known_tags) > 0:
                    highest_tag = int(max(known_tags.itervalues(),
                                      key=lambda nt: int(nt.commit_id))
                                      .commit_id)
                else:
                    highest_tag = -1
                if tags[package_name] > highest_tag:
                    ...
                    for name, commit_id in existing_tags.iteritems():
                        if name not in known_tags:
                            nt = NamedTree(package=package, type='tag',
                                           name=name, commit_id=str(commit_id))
                            add_changed(nt)
                            session.add(nt)
            ...
            for branch_name in branches.iterkeys():
                ...
                if commit_id is not None:
                    ...
                    if nt is None:
                        nt = NamedTree(package=package, type='branch',
                                       name=branch_name,
            ...
            if known_trunk is None:
                if package_name in trunk:
                    nt = NamedTree(package=package, type='branch',
                                   name=None,
                                   commit_id=str(trunk[package_name]))
                    add_changed(nt)
                    session.add(nt)
            else:
...
class Git(VCS):
    ...
    def link(self, package, filename=None,
        ...
        if filename is not None and not directory:
            url = "/{0}.git;a=blob;f={1}".format(urllib.quote(package),
                                                 urllib.quote(filename))
        elif filename is not None and directory:
            url = "/{0}.git;a=tree;f={1}".format(urllib.quote(package),
                                                 urllib.quote(filename))
        else:
            url = ";a=shortlog"
    ...
    def file(self, package, filename, branch=None, tag=None):
        ...
        if branch is not None:
            ...
            extra = ""
        url = "{0}/{1}.git;a=blob_plain;f={2}{3}".format(self.web_root,
                ...
                for type in ('branch', 'tag'):
                    ...
                    for name, nt in nts.iteritems():
                        if name not in known:
                            session.delete(nt)
                if trunk is None:
                    trunk = NamedTree(package=package,
                                      type='branch',
                                      name=None, commit_id=ps['trunk'])
                    add_changed(trunk)
                    session.add(trunk)
...
class GitSsh(Git):
    ...
    def file(self, package, filename, branch=None, tag=None):
        ...
        cmd = ['ssh', 'pet-cat-file', 'cd', cwd, ';', 'git',
               'cat-file', 'blob', '{0}:{1}'.format(tree, filename)]
    ...
    def _summary(self):
        if self._summary_cache is None:
            ...
            if p.returncode != 0:
                raise Exception(
                    "Getting summary for {0} failed: {1}".format(self.root,
                                                                 stderr))
