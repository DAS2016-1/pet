...
class NamedTree(Base):
    ...
    package = sqlalchemy.orm.relation(
        'Package', lazy='joined',
        backref=sqlalchemy.orm.backref('named_trees',  passive_deletes=True))

    def _file(self, filename):
        session = Session.object_session(self)
        return session.query(File).filter_by(
            named_tree=self, name=filename, commit_id=self.commit_id)
    ...
    def link(self, filename, directory=False):
        ...
        return self.package.repository.vcs.link(
            self.package.name, filename, directory, named_tree=self)
...
class WatchResult(Base):
    ...
    named_tree = sqlalchemy.orm.relation(
        'NamedTree',
...
class Wait(Base):
    ...
    named_tree = sqlalchemy.orm.relation(
        'NamedTree',
        backref=sqlalchemy.orm.backref('waits', passive_deletes=True))
