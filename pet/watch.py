...
from pet.exceptions import NotFound, DownloadError, InvalidVersion
from pet.exceptions import RegexpError, WatchException, InvalidWatchFile
...
class WatchRule(object):
    ...
    def parse(self, rule):
        ...
        # When the homepage contains parentheses in the last component use
        # that as pattern.
        # In that case the line has only three (not four) fields so we have to
        # split again.
        try:
        ...
        try:
            ...
            raise InvalidWatchFile(
                "Could not parse regular expression '{0}': {1}."
                .format(pattern, e))
    ...
    def uversionmangle(self, uversion):
        ...
        regexpes = self.options.get('uversionmangle',
                                    self.options.get('versionmangle', None))
    ...
    def dversionmangle(self, dversion):
        ...
        regexpes = self.options.get('dversionmangle',
                                    self.options.get('versionmangle', None))
        if regexpes is not None:
...
class WatchFile(object):
    ...
    def parse(self, watch):
        ...
        message_return = 'Only watch files using version 2 or 3 are supported.'
        self.version = None
        self.rules = []
            ...
            if self.version is None:
                ...
                if match:
                    ...
                    if self.version not in (2, 3):
                        raise InvalidWatchFile(message_return)
            elif line:
                # collect watch rules
                self.rules.append(WatchRule(line))

first_url = '^https?://search.cpan.org/|'
second_url = 'https?://www.cpan.org/|https?://metacpan.org/|'
third_url = 'https?://cpan.metacpan.org/'
_re_cpan_url = re.compile(first_url + second_url + third_url)
...
class Watcher(object):
    ...
    def check(self, watch_file):
        ...
        return dict(version=results[0][1], dversionmangle=results[0][2],
                    homepage=results[0][3], url=results[0][0], errors=errors)

    def check_rule(self, rule):
        try:
            results = None
            if _re_cpan_url.match(rule.homepage):
                results = self._cpan.check(rule.homepage, rule.pattern,
                                           rule.uversionmangle,
                                           rule.dversionmangle)
            # try by hand if url is unknown to cpan
            if results is None:
                results = []
                homepage = _re_sf.sub('http://qa.debian.org/watch/sf.php/',
                                      rule.homepage)
                fh = urlopen(homepage, timeout=TIMEOUT())
                contents = fh.read()
                fh.close()
                if _re_http.match(homepage):
                    # join all groups, only one in
                    # non-empty and contains the link
                ...
                for link in links:
                    ...
                    if match:
                        ...
                        results.append((url, version, rule.dversionmangle,
                                        rule.homepage))
                try:
                    results.sort(key=lambda x: x[1], reverse=True)
                except ValueError:
...
class CPAN(object):
    ...
    def dists(self):
        if self._dists is None:
            dists = []
            module_detail = 'modules/02packages.details.txt.gz'
            contents = self._get_and_uncompress(
                urlparse.urljoin(self.mirror, module_detail))
    ...
    def files(self):
        if self._files is None:
            ...
            # current directory
            interesting = False
            # are we interested in files in the current directory?
            ...
            for line in contents:
                ...
                # lrwxrwxrwx 1 root csc 24 Feb 25 03:20 CA-97.17.sperl ->
                # ../../5.0/CA-97.17.sperl
                fields = line.split()
                if len(fields) < 9:
                    continue
