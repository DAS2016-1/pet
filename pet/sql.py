...
DBUpdater().add(5, statements=[
    ...
    ALTER TABLE named_tree
      ...
      ADD COLUMN last_changed_by TEXT -- maintainer
      name in last changelog entry
    """,
])
...
DBUpdater().add(6, statements=[
    """
    ALTER TABLE named_tree
      ADD COLUMN versions debversion ARRAY NOT NULL
      DEFAULT ARRAY[]::debversion[]
    """,
    """
    ALTER TABLE archive
      ADD COLUMN web_root TEXT NOT NULL
    ...
    """
    ALTER TABLE suite
      ADD COLUMN components TEXT ARRAY NOT NULL
      DEFAULT ARRAY['main', 'contrib', 'non-free']
    """,
    """
...
DBUpdater().add(7, statements=[
    ...
    """INSERT INTO bug_tracker (name, type, url, web_url)VALUES('debian',
      'debianbts', 'http://bugs.debian.org/cgi-bin/soap.cgi',
      'http://bugs.debian.org/')""",
    """
    CREATE TYPE severity
      AS ENUM ('wishlist', 'minor', 'normal',
    ...
    """
    CREATE OR REPLACE
      FUNCTION affects (versions debversion[], found debversion[],
        fixed debversion[])
      RETURNS BOOLEAN
      IMMUTABLE
      STRICT
    ...
    """,
    """
    COMMENT ON FUNCTION affects (versions debversion[], found debversion[],
      fixed debversion[])
      IS 'check if a bug found (fixed) in the given versions affects the
      package with versions "versions"'
    """,
    """
...
DBUpdater().add(8, statements=[
    ...
    ALTER TABLE bug_source
      ADD COLUMN fixed_versions debversion ARRAY NOT NULL
      DEFAULT ARRAY[]::debversion[],
      ADD COLUMN found_versions debversion ARRAY NOT NULL
      DEFAULT ARRAY[]::debversion[]
    """,
])
...
DBUpdater().add(13, statements=[
    ...
    CREATE TABLE watch_result (
      named_tree_id INT PRIMARY KEY NOT NULL REFERENCES named_tree(id) ON
      DELETE CASCADE,
      homepage TEXT,
      ...
      error TEXT,
      last_checked TIMESTAMP(0) WITH TIME ZONE NOT NULL DEFAULT
      CURRENT_TIMESTAMP,
