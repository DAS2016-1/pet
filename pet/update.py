...
from pet.exceptions import FileNotFound
from pet.models import File, Patch, Wait, Session, NamedTree, Package
from pet.models import SuitePackage, Bug, BugSource, WatchResult, Repository
...
class NamedTreeUpdater(object):
    ...
    def delete_old_files(self):
        ...
        self.session.query(File).filter(
            (File.named_tree == self.named_tree) &
            (File.commit_id != self.named_tree.commit_id)).delete()
    ...
    def _get(self, filename):
        ...
        if self.named_tree.type == 'tag':
            contents = self.vcs.file(
                self.package.name, filename, tag=self.named_tree.name)
        elif self.named_tree.type == 'branch':
            contents = self.vcs.file(
                self.package.name, filename, branch=self.named_tree.name)
        else:
            raise ValueError(
                "unknown NamedTree type '{0}'".format(self.named_tree.type))
    ...
    def file(self, filename):
        ...
        try:
            ...
            f = File(
                ...
                commit_id=self.named_tree.commit_id,
    ...
    def update_patches(self):
        ...
        if not changed and not self.force:
            return
        self.session.query(Patch).filter_by(
            named_tree=self.named_tree).delete()
        if patches.contents:
    ...
    def update_control(self):
        ...
        if control_file.contents:
            ...
            if "Uploaders" in control:
                nt.uploaders = [
                    u.strip() for u in control["Uploaders"].split(",")]
            else:
                nt.uploaders = None
    ...
    def update_changelog(self):
        ...
        if changelog_file.contents:
            changelog = debian.changelog.Changelog(
                changelog_file.contents, strict=False)
            ...
            nt.source_changelog = nt.version = None
            nt.distribution = nt.urgency = None
            nt.last_changed = nt.last_changed_by = None
            nt.versions = []
...
class RepositoryUpdater(object):
    ...
    def update_changed_packages(self):
        ...
        try:
            ...
            for nt in self.session.query(NamedTree).join(
                NamedTree.package).filter(
                    Package.repository == self.repository):
                    named_trees_by_package[nt.package].append(nt)
            print "D: Looking for changes in {0} packages.".format(
                len(named_trees_by_package))
            changed = self.vcs.changed_named_trees(
                self.session, named_trees_by_package)
    ...
    def update_all_packages(self):
        for p in self.repository.packages:
            ...
            #   self.session.rollback()
            #   print "E: error while updating package {0}: {1}".format(
            #     p.name, e)
            except:
                self.session.rollback()
                raise
...
class SuiteUpdater(object):
    ...
    def _download(self, source, target):
        ...
        with open(target, 'w') as fh:
            r = subprocess.call(
                ['gzip', '--decompress', '--to-stdout', '--', target_gz],
                stdout=fh)
            if r:
    ...
    def add_package_list(self):
        for component in self.suite.components:
            url = "{0}/dists/{1}/{2}/source/Sources.gz".format(
                self.archive.url, self.suite.name, component)
            target = os.path.join(
                self.tmpdir, "sources-{0}-{1}-{2}".format(
                    self.archive.id, self.suite.id, component))
            self._download(url, target)
            with open(target, 'r') as fh:
                for s in debian.deb822.Sources.iter_paragraphs(fh):
                    if "Uploaders" in s:
                        uploaders = [
                            u.strip() for u in s["Uploaders"].split(",")]
                    else:
                        uploaders = None
                    sp = SuitePackage(
                        suite=self.suite,
                        ...
                        uploaders=uploaders,
                        dsc="{0}/{1}_{2}.dsc".format(
                            s["Directory"], s["Package"], s["Version"]),
                        )
                    self.session.add(sp)
...
class BugTrackerUpdater(object):
    ...
    def _delete_unreferenced_bugs(self, sources):
        ...
        print self.session.query(Bug).join(Bug.bug_sources) \
            .filter(
                (Bug.bug_tracker == self.bug_tracker) & ~
                BugSource.source.in_(sources)) \
            .statement

    def _update_bugs(self, bug_reports):
        bugs = {}
        for bug in self.session.query(Bug).filter_by(
        ...
        for br in bug_reports:
            ...
            try:
                ...
                bug = Bug(
                    bug_tracker=self.bug_tracker, bug_number=br.bug_number)
    ...
    def run(self, named_trees=None):
        ...
        if named_trees is None:
            sources = [s[0] for s in self.session.query(
                NamedTree.source).distinct()]
            self._delete_unreferenced_bugs(sources)
            bug_numbers = [b[0] for b in self.session.query(
                Bug.bug_number).filter_by(bug_tracker=self.bug_tracker).all()]
...
class WatchUpdater(object):
    ...
    def update_watch(self, watch):
        ...
        if result['errors'] is None:
            wr = WatchResult(
                named_tree=watch.named_tree,
                homepage=result['homepage'],
                upstream_version=str(result['version']),
                download_url=result['url'],
                debian_version=result['dversionmangle'](
                    watch.named_tree.version))
        else:
            error = ", ".join([str(e) for e in result['errors']])
            wr = WatchResult(
                named_tree=watch.named_tree,
                homepage=result.get('homepage'), error=error)
        self.session.add(wr)
    ...
    def run(self, named_trees=None):
        ...
        try:
            ...
            named_trees = named_trees.filter(
                (NamedTree.type == 'branch') & (NamedTree.name is None))
            watches = self.session.query(File) \
                .filter(File.name == 'debian/watch') \
                .filter(File.named_tree_id.in_(
            ...
            self.session.query(WatchResult) \
                .filter(WatchResult.named_tree_id.in_(
                    named_trees.from_self(
                        NamedTree.id).subquery())).delete(False)
            for watch in watches:
                print "D: checking watch for {0}".format(
                    watch.named_tree.source)
