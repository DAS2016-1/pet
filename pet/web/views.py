...
class Overview(object):
    ...
    def changelog_url(self, named_tree):
        return route_url(
            'changelog', self.request, named_tree_id=named_tree.id)
...
def changelog(request):
    try:
        named_tree = request.session.query(NamedTree).filter_by(
            id=request.matchdict['named_tree_id']).one()
    ...
    changelog = debian.changelog.Changelog(
        changelog_contents, max_blocks=1, strict=False)
